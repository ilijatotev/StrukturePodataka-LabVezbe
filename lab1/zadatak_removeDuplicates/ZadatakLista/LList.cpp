...
void LList::removeDuplicates()
{
	...
	for (LListNode* curr = head; curr != nullptr && curr->next != nullptr; curr = curr->next) {

		LListNode* prev = curr;
		LListNode* nextEl = nullptr;
		for (nextEl = curr->next; nextEl != nullptr;) {

			if (curr->isEqual(nextEl->info)) {
				LListNode* tmp = nextEl;
				nextEl = nextEl->next;
				prev->next = tmp->next;
				delete tmp;
			}
